...
class BonusTableNotifier extends StateNotifier<BonusTableState> {
  ...
  Future<void> loadInitialData() async {
    try {
      ...
      final listItems = calculateBonusItems(
        bonuses,
        targetRatio,
        overtimeRatio,
        overtimeHours,
  ...
  Map<String, dynamic> getBonuses() {
    ...
    if (check) {
      return ayrBonusPercentageMap.map(
        (key, value) => MapEntry(value.toString(), key),
      );
    } else {
      return seasonalBonusPercentageMap.map(
        (key, value) => MapEntry(value.toString(), key),
      );
    }
  }
    ...
    if (targetRatio * 100 >= 212.5) {
      ...
      for (final key in sortedKeys) {
        ...
        if (requiredAmount > 0) {
          ...
          bonusData.add(
            BonusItem(bonus: bonus, requiredAmount: requiredAmount),
          ); // Collect each item
        }
      }
    }
    state = state.copyWith(bonusData: bonusData);
...
final bonusTableProvider =
    StateNotifierProvider.autoDispose<BonusTableNotifier, BonusTableState>(
